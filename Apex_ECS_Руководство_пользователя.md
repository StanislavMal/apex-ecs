...
**DoD-сцена движка `many_foxes`** — стресс скиннинга (N анимированных Fox.glb в кольцах через
`ChildOf`-иерархию: `AnimationPlayer × N` → joint-палитры → GPU-скины + тени). Замер:
`APEX_FOXES=N APEX_UNCAPPED=1 cargo run --release -p apex-examples --example many_foxes`
(i5-12400F + RTX 4060 Ti, release + LTO, uncapped/без vsync, медиана steady-state после прогрева;
...
> @1000 держит **~460 FPS** (i5-12400F/RTX 4060 Ti). Прежняя веха кампании CR-M — ~160→~390 FPS
> (Bevy на той же сцене ~235); текущий прирост над ней — cost-model-планировщик + перф-фиксы после.
> Bevy/Legion в этом прогоне не перемерялись — свежее число только для apex; относительное сравнение
> с эталонами — микро-бенчи §14.8.
