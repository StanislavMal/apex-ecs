...
> **«Запуск = прочтение» (ADR-010).** Для ДЕКЛАРИРОВАННЫХ ридеров систем (параметр
> `EventReader<E>`, событийный параметр `system!`, `Listen<E>`) курсор продвигается до конца
> буфера в конце КАЖДОГО запуска системы — даже если тело сделало ранний `return` и не вызвало
> `read()` (запуск = использованный шанс; поведение идентично «прочитал и проигнорировал»).
> «Дренить» ридеры на ранних выходах не нужно — система, которая запустилась, не может
> удержать retention. No-loss-удержание работает ровно для систем, которые планировщик
> **не запускал** (run-condition, выключенная стадия): их курсор стоит, события ждут — при
> пробуждении система получит всё. «Хочу накапливать» выражается декларативным гейтом, а не
> ранним `return`; осознанное частичное чтение — `Events::read_partial`/`advance_reader_by`
> (queue-уровень, владелец сам несёт удержание). Буфер, удерживаемый ≥64 тиков подряд, громко
> предупреждает в лог (`Events::retained_ticks()` — диагностический аксессор).
...
- ✅ **Спайки в пределах эскроу — тоже детерминированы.** Каждый сеемый блок = адаптивный размер
  (пик × 2) плюс **эскроу-margin** (половина блока) — приватный rank-детерминированный хвост. Спайк,
  превысивший адаптивный блок, тянет детерминированные id из эскроу вместо общего пути. Так гарантия
  покрывает не только steady-state, но и реалистичные спайки (примерно до 3× наблюдённого пика).
  Неиспользованный эскроу возвращается в reuse-пул (id-пространство ограничено).
- ⚠ **Граница — спайк за пределами блок+эскроу:** если система за кадр спавнит больше, чем вместили
  блок И эскроу (очень крупный всплеск), избыток берётся из общего пути — недетерминирован **тот
  кадр**. Но это теперь **громко**: печатается предупреждение и растёт счётчик
  `Scheduler::deterministic_overflow_count` (не молчаливая деградация). Следующий кадр блок растёт и
  детерминизм восстанавливается. Бесконечный детерминированный overflow сознательно не реализован —
  эскроу покрывает практические спайки.
...
### 8.1a Порядок детей (sibling order) — гарантия и управление (ADR-008)

Список subjects каждой пары `(kind, target)` **упорядочен**, и порядок — публичная
гарантия для ВСЕХ видов связей:

- `add_relation` добавляет в КОНЕЦ; удаление сохраняет порядок остальных;
- `targets_of` выдаёт детей ровно в этом порядке;
- snapshot/restore воспроизводит порядок точно (см. §10);
- эксклюзивный re-parent (`ChildOf` на нового родителя) добавляет в конец списка
  нового родителя.

Явное управление порядком (UI z-order, «переместить ребёнка вверх/вниз» в
редакторе, детерминированная сборка сцены):

```rust
// Вставить/переставить на позицию (clamp по длине списка):
world.insert_relation_at(child, ChildOf, parent, 0);      // ensure-at: нет связи — создаст,
                                                          // есть — чистая перестановка
let moved = world.set_relation_index(child, ChildOf, parent, 2); // только перестановка;
                                                          // false, если связи нет (не создаёт)
let pos = world.relation_index(child, ChildOf, parent);   // -> Option<usize>
```

Перестановка — чистое изменение порядка: хуки `on_relation_add/remove` НЕ
вызываются (набор рёбер не меняется).

> **Исключение (осознанное):** `query_relation`/`query_wildcard` порядок детей НЕ
> сохраняют — они перегруппировывают subjects по архетипам ради скорости выборки.
> Нужны и порядок, и данные — итерируйте `targets_of` + точечный `get`.
...
Наблюдатели за появлением/исчезновением/перестановкой пар вида `R`. На каждый вид и событие
можно регистрировать **несколько** хуков (fan-out) — они вызываются в порядке регистрации
(например, UI-раскладка и редактор независимо наблюдают `ChildOf`):
...
world.on_relation_reorder::<ChildOf>(|w, subject, target| {
    /* ЧИСТАЯ перестановка среди сиблингов (§8.1a): set_relation_index или
       insert_relation_at существующей пары; add/remove при этом НЕ вызываются
       (набор рёбер не изменился) — новая позиция: w.relation_index(...) */
});
```
...
стоимость на горячих путях `add_relation`/despawn (один bool-гейт). Отличие от хуков
компонентов: у `on_add::<T>` — один хук на тип (fan-out через события), у relation-хуков —
несколько (у иерархии несколько независимых подписчиков — доказанный кейс).
...
### 14.0 Где мы стоим относительно эталонов (замер 2026-08-30)

Полный прогон `tools/bench_baseline.ps1 -Run "."` на коммите `274b09b`, чистое дерево, одна и та
же машина, criterion (медиана 100 выборок). **Отношение больше 1× значит, что мы БЫСТРЕЕ**: это
время эталона, делённое на наше.

**ВПЕРЕДИ эталона.**

| группа | apex | bevy | legion | vs bevy | vs legion |
|---|---|---|---|---|---|
| `changed_iter_static` | 14.1 нс | 3.4 мкс | — | **240.37×** | — |
| `changed_iter_frag` | 432.2 нс | 4.1 мкс | — | **9.57×** | — |
| `despawn_recursive` | 21.7 мкс | 49.8 мкс | — | **2.30×** | — |
| `events_frame_loop` | 283.8 мкс | 618.8 мкс | — | **2.18×** | — |
| `events` | 12.5 мкс | 26.5 мкс | — | **2.12×** | — |
| `commands_spawn` | 223.2 мкс | 447.4 мкс | — | **2.00×** | — |
| `add_remove_component` | 514.6 мкс | 795.4 мкс | 2447.3 мкс | **1.55×** | 4.76× |
| `schedule` | 30.7 мкс | 39.3 мкс | 30.4 мкс | **1.28×** | 0.99× |
| `relations` | 565.7 мкс | 607.9 мкс | — | **1.07×** | — |
| `get_component` | 32.3 мкс | 34.4 мкс | 47.5 мкс | **1.06×** | 1.47× |
| `commands_insert` | 438.0 мкс | 459.6 мкс | — | **1.05×** | — |
| `spawn_wide` | 621.2 мкс | 628.7 мкс | — | **1.01×** | — |
| `despawn` | 270.8 мкс | 272.9 мкс | 443.2 мкс | **1.01×** | 1.64× |
| `simple_iter` | 8.5 мкс | 8.5 мкс | 5.6 мкс | **1.00×** | 0.66× |
| `propagate` | 256.5 мкс | — | — | — | — |
| `propagate_static` | 99.6 нс | — | — | — | — |

**ПОЗАДИ эталона.**

| группа | apex | bevy | legion | vs bevy | vs legion |
|---|---|---|---|---|---|
| `fragmented_iter` | 177.0 нс | 126.8 нс | 168.0 нс | **0.72×** | 0.95× |
| `events_frame_idle` | 46.5 мкс | 34.8 мкс | — | **0.75×** | — |
| `simple_insert` | 385.6 мкс | 306.1 мкс | 211.2 мкс | **0.79×** | 0.55× |
| `changed_iter` | 8.0 мкс | 6.9 мкс | — | **0.85×** | — |
| `heavy_compute` | 545.4 мкс | 492.3 мкс | 406.2 мкс | **0.90×** | 0.74× |
| `wide_iter` | 3.4 мкс | 3.4 мкс | 2.1 мкс | **0.97×** | 0.62× |

**Как эту таблицу читать — три оговорки, без которых она врёт.**

1. **`changed_iter_static` (240×) и `changed_iter_frag` (9,6×) — не трофей, а разные механики.**
   У нас выборка, в которой ничего не менялось, отвечает по счётчику тиков и не обходит данные
   вовсе; у эталона такой ранний выход отсутствует, и он платит обход. Число честное, но оно
   измеряет наличие механизма, а не то, что «мы в 240 раз лучше пишем циклы».
2. **Длинный набор греет машину.** `heavy_compute/bevy` в полном прогоне читается на ~19 % выше,
   чем в отдельном (проверено тремя прогонами подряд 2026-08-30: 467/474/471 мкс против 554 мкс
   в наборе). Поэтому отдельные строки полного прогона стоит перепроверять точечным запуском,
   прежде чем делать из них вывод.
3. **Одно число — не вердикт.** Стенд судит по своему правилу (регресс = медиана хуже на 20 %) и
   сам отказывается судить группу, чей эталон нестабилен. Числа выше — снимок, а не гарантия.

**Где мы уверенно впереди:** удаление поддерева, события (и их пустой кадр), запись `Commands`,
добавление/снятие компонента, планировщик. **Где позади:** чистая итерация по плотным данным
(`simple_iter`/`wide_iter` против legion), вставка (`simple_insert`), фрагментированный обход и
`heavy_compute`. Отставание на итерации — известное и названное: у legion другая раскладка
хранилища, и это предмет отдельной линии, а не забытая мелочь.

⚠ Таблица снимается СТЕНДОМ и живёт в `tools/baselines/core_bench.baseline.json`. Если правки
ядра меняют числа — переснимите байзлайн (`-Record`, только на чистом дереве: стенд откажется
записать файл, который назвал бы коммит, которого он не мерил) и обновите этот раздел ТЕМ ЖЕ
коммитом.
...
Числа ниже — **полный 3-way прогон 2026-08-27** (все движки в одном процессе ⇒ дрейф машины
канселится, ratio apex/bevy надёжен; абсолютные значения ±10-40 % между прогонами). Прогон снят
на фиксе `BENCH-REGRESS-0824` (реестр `plans/TECH_DEBT.md`) — том самом заходе, который вернул
`schedule`/`wide_iter`/`fragmented_iter` к записанным числам. Машина: Windows 11, 12 логических
ядер. Снимок этих же чисел закоммичен как байзлайн-гейт
(`tools/baselines/core_bench.baseline.json` + `tools/bench_baseline.ps1`).

> **Как читать абсолютные числа.** Между двумя прогонами ОДНОГО дня на этой машине эталоны
> (bevy/legion) разошлись на 2-11.5 %. Поэтому колонка «Итог» (отношение) — надёжна, а абсолютные
> микросекунды сравнимы только внутри одного прогона. Именно поэтому компаратор байзлайна смотрит
> на эталоны ПЕРВЫМИ и отказывается сравнивать, если они уехали: тогда прогон говорит о машине,
> а не о ядре.

| Бенч | apex | bevy 0.18 | legion 0.4 | Итог vs Bevy |
|------|:----:|:---------:|:----------:|------|
| changed_iter_static (10k, НИЧЕГО не изменилось) | **15 ns** | 3.54 µs | — | 🟢 **×241** |
| changed_iter_frag (26 арх, Changed) | **473 ns** | 4.29 µs | — | 🟢 **×9.1** |
| despawn_recursive (поддерево 1k) | **22.4 µs** | 52.4 | — | 🟢 **×2.3** |
| events (send+read 10k) | **13.4 µs** | 29.2 | — | 🟢 **×2.2** |
| add_remove (10k) | **520 µs** | 832 | 2627 | 🟢 ×1.6 (vs legion ×5.1) |
| schedule (3 sys / 40k) | **26.3 µs** | 39.5 | 30.7 | 🟢 **×1.5** (и ×1.16 vs legion) |
| commands_spawn (10k) | **398 µs** | 457 | — | 🟢 ×1.15 |
| get_component (random ×10k) | **34.9 µs** | 37.9 | 50.6 | 🟢 ×1.09 (vs legion ×1.45) |
| despawn (10k) | **280 µs** | 295 | 474 | ≈ паритет (vs legion ×1.7) |
| or_iter | **5.01 µs** | 5.16 | — | ≈ паритет |
| simple_iter (10k) | 8.74 µs · dense **6.38** | 8.67 | 5.76 | ≈ паритет (dense ×1.36) |
| wide_iter (5 comp: 4R+1W) | 3.69 µs | 3.66 | **2.13** | ≈ паритет |
| commands_insert (10k) | 510 µs | **467** | — | 🟡 −8 % |
| heavy_compute (par) | 548 µs | **506** | **410** | 🟡 −8 % |
| changed_iter (Changed, 10 % dirty) | 8.47 µs | **7.56** | — | 🟡 −11 % |
| simple_insert (10k×4 comp) | 371 µs | **329** | **227** | 🟡 −11 % |
| events_frame_loop (кадровая петля) | 134 µs | **107** | — | 🔴 −20 % |
| relations (build+iter 10k ChildOf) | 855 µs | **622** | — | 🔴 −27 % |
| fragmented_iter (26 арх) | 175 ns | **126** | 174 | 🔴 −28 % (≈ legion) |

Группы без аналога у эталонов (apex-фокус): `propagate` 281 µs, **`propagate_static` 103 ns**
(10k узлов, ничего не движется — PE-C1), `query_relation` 43.9 µs, `par_split`/`par_split_ab`/
`par_uniform_ab` (A/B диспетчера, `decisions/ADR-003`).

**Итог: 8 побед / 4 паритета / 7 отставаний** против современного Bevy. Главное изменение с
прошлой редакции таблицы: **`schedule` перевернулся в победу ×1.5** (cost-model планировщик,
§13.1.3 и `ADR-003`), а **семейство `changed_*` стало нашим самым большим отрывом** — ×241 на
статичной сцене и ×9.1 на фрагментированной, потому что поколонный агрегат тиков (`ADR-011`)
позволяет `Changed<T>` скипать целые архетипы за O(1), тогда как Bevy сканирует построчно. Это
не микро-тюнинг, а другая асимптотика, и она — ровно про то, как выглядит кадр редактора:
статичная сцена с живыми потребителями изменений.

**Природа отставаний (не баги):**
- **fragmented_iter** — 26 архетипов по 20 строк: доминирует пер-архетипная работа, а не итерация.
  Мы вровень с Legion и позади Bevy на микро-тюнинге его codegen. Алгоритмически обход тот же.
- **relations** — архитектурная цена ДВУСТОРОННЕГО индекса (у Bevy его нет; за это мы берём
  `despawn_recursive` ×2.3 и `query_relation`, которых у него нет вовсе).
- **simple_insert / heavy_compute / commands_insert** — диффузные 8-11 %: bound на archetype-move
  из разбросанных строк и микро-тюнинг codegen. Где Legion впереди
  (simple_insert/simple_iter/wide_iter/heavy_compute) — у него **нет change detection**; iter-выигрыш
  Legion apex уже берёт своим dense-путём (`for_each_chunk`: simple_iter dense 6.38 ≈ legion 5.76).
- **changed_iter (10 % dirty)** — на «горячей» сцене агрегат не спасает (архетип не скипается), и
  остаётся построчный скан; −11 % — цена нашего более дорогого `Mut`. Обратная сторона тех самых
  ×241/×9.1 на холодной и фрагментированной сцене.
- **events_frame_loop** — единственная событийная группа, где мы позади (при том что `events`
  ×2.2): кадровая петля платит за персистентные курсоры ридеров (`ADR-010` «запуск = прочтение»),
  которых у Bevy нет. Осознанная цена семантики, не дефект.
...
**Назначение — непроизводительные элементы.** Одна Lua-VM однопоточна внутренне
(`ScriptEngine` привязан к потоку создания, `!Send`). Он идеален для событийно-
ориентированной логики (диалоги, квесты, триггеры), тюнинга параметров и быстрого
прототипирования. Для CPU-bound обработки тысяч сущностей оставайтесь на чистых
Rust-системах (`AutoSystem`).

**Две модели запуска:**
- **Монолитный `run()`** (§17.1–17.8) — весь скрипт исполняется как одна
  эксклюзивная операция вне планировщика (`engine.run(dt, &mut world)`).
- **Lua-системы `system{}`** (§17.9) — отдельные скрипт-функции регистрируются как
  **первоклассные системы планировщика** (`engine.register_systems(&mut scheduler)`).
  Несмотря на однопоточность VM, они **исполняются конкурентно с непересекающимися
  Rust-системами** (main-thread NonSend-вид), с декларированным доступом,
  детерминированными спавнами и hot-reload. Это золотой путь для игровой логики.
...
-- Changed — реактивный фильтр: компонент изменён с прошлого прогона (значение — через Read:)
-- Added   — реактивный фильтр: компонент добавлен с прошлого прогона (значение — через Read:)
...
query({"Read:Position", "Changed:Position"})             -- только изменённые Position
...
`Changed:`/`Added:` — реактивные фильтры (симметрично `With:`); чтобы получить значение
отфильтрованного компонента, добавьте `Read:` того же типа. Проходят через ту же ядерную
`DynQuery`-поверхность, что и типизированные запросы (§10.8).
...
### 17.9 Lua-системы как первоклассные системы планировщика (`system{}`)

Вместо одного монолитного `run()` скрипт может объявить **несколько отдельных
систем**, которые планировщик запускает как обычные системы — с конфликт-детекцией,
детерминизмом и hot-reload. Это золотой путь для игровой логики на Lua.

```lua
-- gameplay.lua — объявления верхнего уровня (исполняются при register_systems)
system{
    name  = "integrate",
...
```rust
// Регистрация и запуск через планировщик
let mut scheduler = Scheduler::new();
engine.register_systems(&mut scheduler);   // транслирует system{} → системы планировщика

loop {
    engine.poll_hot_reload();               // при изменении .lua-файла
    engine.register_systems(&mut scheduler);// идемпотентно: заменяет прошлое поколение
    engine.set_delta_time(dt);              // dt для всех Lua-систем этого кадра
    scheduler.run(&mut world);              // Lua-системы бегут вместе с Rust-системами
    world.tick();
}
```

**Модель исполнения и параллелизм.** Каждая Lua-система — main-thread `NonSend`-система
(VM однопоточна). Все Lua-системы декларируют скрытую запись на маркер-токен `ScriptVm`,
поэтому планировщик **сериализует Lua↔Lua** (одна VM никогда не трогается двумя системами
разом) и при этом **параллелит Lua↔Rust** по реальным декларациям доступа: Lua-система с
`Write:Position` не пойдёт параллельно с Rust-системой, пишущей `Position`, но пойдёт
параллельно с независимой. Эксклюзивность выражена в access-модели, без `Mutex`.

**Декларированный доступ (обязателен).** `query` в `system{}` — это декларация доступа для
планировщика. Внутри `fn` вызовы `query()`/`commit()` резолвят **только** объявленные
компоненты; обращение к необъявленному — громкая аномалия (§0.2a) и пустой результат.
Система, ссылающаяся на **незарегистрированный** компонент, **не регистрируется вовсе**
(иначе недо-декларация = гонка). Регистрируйте компоненты до `register_systems`.

**Эффекты.** Полный набор доступен и применяется через per-system Commands-слот
планировщика после стадии: `commit` (запись компонентов), `spawn_entity`/`despawn`
(структурные — **детерминированные id** под `scheduler.set_deterministic_spawn(true)`),
`write_resource`/`emit_event` (ресурсы и события).

**Hot-reload.** `register_systems` идемпотентен: повторный вызов после reload сначала
**снимает** прошлое поколение скрипт-систем из планировщика, затем регистрирует текущее —
изменённый набор `system{}` заменяет старый без дублей.

**`system{}` vs монолитный `run()`:**

| | `run()` (§17.1) | `system{}` (§17.9) |
|---|---|---|
| Запуск | `engine.run(dt, &mut world)` вне планировщика | `scheduler.run(&mut world)` |
| Конкуренция | эксклюзивно, одна операция | параллельно с непересекающимися Rust-системами |
| Гранулярность | один скрипт = одна операция | много именованных систем на скрипт |
| Детерминизм спавнов | id аллокатора | детерминированные id (D8b) |
| Конфликт-детекция | нет (эксклюзив) | по декларациям + токен `ScriptVm` |
