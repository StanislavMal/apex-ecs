...
- ✅ **Гарантируем: single-binary детерминизм** — один и тот же бинарь при одинаковом старте + входах
  воспроизводит идентичное присвоение id, идентичный порядок применения команд и iteration-independent
  результат. Достаточно для record/replay и rollback на одной машине/бинаре.
...
- ⚠ **Вне scope (фронтир):** детерминированное **переиспользование** освобождённых слотов под тяжёлым
  despawn+respawn churn (блоки карвят свежие id из high-water, не из free-list → под churn id-пространство
  растёт). Opt-in трейдит per-frame id-пространство за детерминизм.
