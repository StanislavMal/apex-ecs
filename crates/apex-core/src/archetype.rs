...
pub(crate) struct Column {
    ...
    pub(crate) len: usize,
    pub(crate) capacity: usize,  // ← pub(crate)
}
...
impl Column {
    ...
    pub unsafe fn swap_remove_and_drop(&mut self, row: usize) -> bool {
        ...
        if row != last {
            ...
            if self.item_size > 0 {
                let last_ptr = self.get_ptr(last);
                std::ptr::copy_nonoverlapping(last_ptr, remove_ptr, self.item_size);
            }
            self.len -= 1;
            true
        } else {
            let ptr = self.get_ptr(row);
            (self.drop_fn)(ptr);
            self.len -= 1;
            false
        }
    ...
    pub(crate) fn grow(&mut self) {  // ← pub(crate)
