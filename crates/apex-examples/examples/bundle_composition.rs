...
fn main() {
    ...
    {
        // Speed напрямую + PlayerBase + Armor — порядок не важен.
        // ВАЖНО: каждый тип компонента в бандле должен быть УНИКАЛЕН —
        // дубликат (напр. Hp напрямую И внутри PlayerBase) паникует.
        let e = world.spawn((
            Speed(20.0),
            PlayerBase {
