...
- **`World::increment_change_tick() -> Tick`** (Bevy 1:1) — текущий тик с закрытым за ним окном.
  Читатель, отмечающий базу `last_run`, берёт её этим вызовом, и всякая запись после отметки строго
  новее неё. `propagate_transforms` отмечала базу `current_tick()` и часы не двигала: сущность,
  заспавненная или сдвинутая на том же тике после прямого вызова (проход обслуживания редактора,
  правка между кадрами), не получала `GlobalTransform` ни на следующем проходе, ни на каком
  (apex-engine TD-565: три кадра без мирового трансформа, picking бросал луч из начала координат).
  Внутри расписания дыры не было — окна стадий двигают часы между системами. Гейт
  `a_transform_written_right_after_a_pass_is_propagated_by_the_next`; инъекция возврата
  `current_tick()` даёт `None` вместо `(0, 2, 0)`.
...
- **`World::insert_resource_with_tick(value, tick)`** — вернуть ресурс в мир с ТЕМ тиком, с
  которым его забрали, вместо текущего. У обычного `insert_resource` семантика «значение
  пришло»: новое значение — это изменение, и всякий читатель, гейтнутый на тик, обязан
  перечитать. У ЗАЙМА её не было вовсе. Скобка, которая забирает ресурс из мира на время
  вызова (чтобы `&mut World` внутри не алиасил его) и кладёт ТО ЖЕ значение обратно, не
  изменила ничего — а insert говорил, что изменила, и читатель, вся работа которого состоит
  в том, чтобы пропускать кадры, пока значение стоит, перестраивался оттого, что на мир
  кто-то ПОСМОТРЕЛ. Цена этого в движке измерена: у редактора asset-скобка стоит на дороге
  каждого inspect, и перетаскивание гизмо роняло кадр вьюпорта с 9,5 до 30 мс (apex-engine
  TD-530). Решение о том, был ли заём записью, остаётся у вызывающего: писал — клади обычным
  insert. Гейт `a_resource_put_back_as_a_loan_does_not_look_changed` держит обе половины
  (заём тик не двигает, запись двигает), инъекция «заём штампует текущий тик» даёт Tick(3)
  против Tick(1).

### Fixed — приложение становилось МЕДЛЕННЕЕ после массового удаления (2026-08-06)

- **`EntityAllocator::flush` пересобирал аренду на каждом холостом вызове.** Быстрый путь
  проверял `free_list.is_empty() && lease.free.is_empty()` — и переставал срабатывать
  навсегда, как только хоть одна сущность была удалена: освобождённые индексы с этого
  момента живут в аренде, поэтому каждый следующий flush проходил весь пул и выделял новый
  `Vec` + `Arc`. `Commands::apply` зовёт `World::flush_reserved` для КАЖДОГО командного
  буфера КАЖДОЙ стадии КАЖДОГО кадра ⇒ установившаяся покадровая цена была пропорциональна
  числу УДАЛЁННЫХ сущностей: приложение тормозило тем сильнее, чем больше объектов удалили
  (найдено пользователем в редакторе после удаления нескольких сотен моделей). Условие
  дополнено «аренда не тронута» (`unconsumed == lease.free.len()`) — тогда возврат+переиздача
  дали бы байт-идентичную аренду, и их можно пропустить. **20k освобождённых: 87.4 мкс →
  0.026 мкс на холостой flush (×3400).** Регресс-тест `idle_flush_does_not_rebuild_the_lease`
  (структурный: `Arc::ptr_eq` — холостой flush не переиздаёт аренду).
- **Планировщик применял командные слоты сверх текущей стадии** (следствие PE-C4-пула):
  пул хранит слоты самой широкой стадии, а цикл применения шёл по всему вектору — лишние
  `flush_reserved` для буферов, в которые ни одна система записать не могла. Теперь строго
  живой префикс `[..stage_ids.len()]`.
