...
- **string-table снапшота 🟢 ОТКРЫТ (переоценён 2026-07-06, CORE_POLISH волна 0.6 → отдельный
  focused-заход) — `type_name` per-instance.** `serializer.rs` пишет `info.name.to_string()` на каждый
  ...
  тест-план — в плане кампании §0.6. Не полумера, а честный перенос полного решения в соразмерный
