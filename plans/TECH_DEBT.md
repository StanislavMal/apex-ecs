...
## 🟡 Блок движкового аудита PERF_ECONOMY (2026-08-06; ключи PE-C*)

> Происхождение: четырёхсторонний код-аудит движка 2026-08-05 (кампания PERF_ECONOMY,
> `apex-engine/plans/…/PERF_ECONOMY.md` §1) вскрыл ядровой пласт «покадровая цена не
> пропорциональна изменённому». По решению кампании (§4 не-скоуп) пласт НЕ чинился движковым
> заходом — это правки хранилища/планировщика со своими бенчами и Miri-гейтами.
> **Предложение: кампания CORE_ECONOMY** — самый большой множитель здесь PE-C2 (поколонный
> агрегат тиков дешевит разом экстракт движка, propagate и ВСЕ Changed-системы обоих репо).

- **PE-C1 🟡 — тик-скан всех `LocalTransform` до раннего выхода** (`transform.rs`,
  changed-фаза `propagate_transforms`): линейный проход тик-колонок всех строк с трансформом
  каждый кадр, даже когда изменённых ноль. Лечится PE-C2 (поколонный агрегат) почти даром.

- **PE-C2 🟡 — нет поколонного `max_change_tick` на колонку/архетип ⇒ каждая `Changed<T>`-система
  O(строк с T) на кадр.** Скан дешёвый (сравнение тика), но множится на ЧИСЛО Changed-потребителей
  (экстракт движка: entity-sync, скины PE.6, пикинг…). Агрегат «max тик колонки в архетипе»,
  поддерживаемый при записи, срезает статические архетипы за O(архетипов). Смежно движковому
  TD-207 (там — потребительская сторона). Ядро самого большого выигрыша CORE_ECONOMY.

- **PE-C3 🟢 — `any_with_component` через `count()`**: полный подсчёт вместо раннего выхода по
  первому совпадению (архетипный `is_empty`-чек — O(архетипов)).

- **PE-C4 🟢 — арены команд дропаются вместо переиспользования** (`executor.rs`): аллокация арен
  на кадр; держать пул/переиспользовать буферы (паттерн уже есть у event-блоков D8b).

- **PE-C5 🟢 — identity-вектора `all_indices` ×5**: несколько мест строят вектор `0..n` заново
  каждый кадр; кэш или итерация без материализации.

- **PE-C6 🟡 — `resource_mut` штампит тик ПРИ ВЗЯТИИ (не при записи), нет `ResMut::set_if_neq`,
  планировщик каждый кадр помечает `FixedTime` изменённым.** Следствия: `Changed`-гейты по
  ресурсам срабатывают на доступ-без-правки (движковый PE.6-кэш prepare() это учитывает —
  консервативно, но лишние ребилды), а `FixedTime` держит вечную «изменённость». Канон Mut-модели
  ядра уже умеет `set_if_neq` для компонентов (IDLE_POWER) — распространить на ресурсы и
  штамповать по фактической записи (deref_mut), как у компонентного `Mut`.

- **PE-C7 🟢 — док-фикс: rustdoc-цифры интервала `check_change_ticks` не сходятся с формулой**
  (аудит: фактический интервал ~39 ч, в доке «3 дня»). Пересчитать от `TICK_CHECK_INTERVAL =
  1<<26` и фактической частоты тиков и поправить rustdoc (формулой, не константой) — интервал
  сам по себе корректен (`MAX_CHANGE_AGE = 1<<30` даёт большой запас, W2-3).
