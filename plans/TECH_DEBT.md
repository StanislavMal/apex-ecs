...
- **BENCH-REGRESS-0824 🟡 — незамеченные перф-регрессы ядра: schedule ×2.5, wide_iter ×2.2,
  fragmented_iter ×2.06 к собственным записанным числам; запись «No change» уже сравнивала с
  регрессировавшим байзлайном.** Найдено 2026-08-24 свежим прогоном бенчей (criterion, features
  ...
  **План:** (1) бисекция по трём группам в окне; вернуть ≥ записанных чисел; (2) закоммиченный
  байзлайн-снимок (JSON: группа → медиана, машина, коммит) + скрипт сравнения с порогом —
  локальный гейт перед пушем ядра, CI-фри (шумные раннеры — политика не меняется); (3) обновить
  §14.8 руководства по свежему стендингу (там до сих пор прогон 2026-06-17). Движковый контекст:
