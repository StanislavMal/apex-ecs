...
## Замер и сверка с эталонами 2026-08-30 (по запросу пользователя)

Пользователь спросил прямо: делался ли ПОЛНЫЙ сравнительный прогон после последних правок, и
актуальны ли числа в руководстве. Ответ был «нет» на оба, и вот что сделано.

**1. Полный прогон.** `bench_baseline.ps1 -Run "."` на чистом дереве. Регрессий нет среди судимых
групп; `heavy_compute` в наборе не судился — у него уехал СОБСТВЕННЫЙ эталон (bevy 465→554 нс).
Проверено отдельно тремя прогонами подряд: apex 539,8/538,5/543,8 мкс (разброс 1,0 %), bevy
467,4/473,8/470,9 (1,4 %), legion 408,7/402,6/402,2 (1,6 %). То есть бенчмарк стабилен, а 554 нс
поймал ДЛИННЫЙ набор — машина к концу прогона другая. Группа судима, `heavy_compute/apex` ровно.

**2. Дефект стенда, вскрытый этим вопросом (коммит `274b09b`).** Стенд читал КАТАЛОГ criterion, а
не прогон: удалённые бенчмарки (`or_iter`, `query_relation`) отвечали годами, а любой
отфильтрованный прогон печатал полную таблицу, где незапущенное читалось «0,0 % — без изменений».
Заголовок «no regression among the 24 judged» опирался на 21 измерение и три призрака. Теперь
несвежее чтение = отсутствующее (стенд называет исключённые поимённо), а `-Record` отказывает на
грязном дереве — прежний байзлайн был помечен `97a58f9`, хотя мерил код `11c3375`.

**3. Байзлайн переснят** на `274b09b`: 23 группы, 11 призраков не переехали.

**4. Руководство актуализировано** — §14.0 «Где мы стоим относительно эталонов»: полная таблица
apex/bevy/legion с тремя оговорками (240× у `changed_iter_static` — разные механики, а не наш
цикл; длинный набор греет машину; одно число не вердикт).

**5. Движок после правок ядра** (`perf_gate.ps1 -Legs all`): регрессий нет. moving 167,52→161,39
fps (−3,7 %), static 659,44→652,51 (−1,1 %), редактор −1,9…−8,4 % по тикам. Всё чуть ниже
байзлайна и в ядре, и в движке одинаково — это машина сегодня, а не правки. И предсказано заранее:
правки ядра трогают CPU-сторону, а `moving` упирается в рендер-тик (TD-337), где спавн и очередь
команд не бегут вовсе.
...
## Внеплановая работа 2026-08-30 (вторая) — ПРИБОР ПРЕЖДЕ МЕХАНИКИ: две лестницы атрибуции

Заход начат с решения пользователя: сначала два дешёвых прибора, а не механика. Обе строки —
семья спавна и `commands_insert` — стояли «правдоподобно, но непроверено»: гипотезы были названы,
но ни одна не измерена. Прибор обязан дать один из ДВУХ равно ценных ответов — либо назвать
предмет числом, либо показать, что зазор в самом приборе. Он дал первый, дважды, и попутно
опроверг обе гипотезы, которые план считал главными.

### Что за приборы

`apex-bench --bin spawn_ladder` и `--bin commands_ladder` (оба CPU-only). Лестница внутри
производственного тела: проход на ступень, каждая ступень исполняет на один этап больше;
читаются РАЗНОСТИ. Форма взята у движкового `APEX_EXTRACT_LADDER`, описание —
`apex-engine/docs/TESTING.md`, реестр — LADDER-SPAWN-0830 / LADDER-CMD-0830.

**Три дефекта самих приборов, найденные и починенные ДО того, как их числами воспользовались** —
записаны, потому что каждый из них однажды уже стоил бы захода:

1. **Ступени мерились блоками.** Первая версия давала ОТРИЦАТЕЛЬНЫЕ разности при разбросе до 66 %:
   дрейф машины между минутами доставался той ступени, которая в этот момент бежала. Чередование
   ступеней внутри окна — и разбросы стали 2–14 %. (Тот же урок, которым живёт `ab_bench.ps1` в
   движке; лестница его не унаследовала, потому что писалась как «просто несколько прогонов».)
2. **Копия расходилась с оригиналом на 20 %** на ширине 8: копия обходила колонки ОДИН РАЗ НА
   СТУПЕНЬ, а производство интерлейвит их per-component. Сведено к трём проходам (рост ёмкости
   обязан идти до generic-записи данных, учёт — после), ступени выше записи переключаются ВНУТРИ
   одного прохода учёта. Дрейф ≤4 %. Если бы эту строку не печатали, разности ширины 8 читались бы
   как находка о ядре, а были бы находкой о копии.
3. **Сток данных инвертировал порядок ступеней.** В половине apply ступень 0 печаталась ДОРОЖЕ
   ступени 1, которая делает строго больше: `black_box` кортежа на ступени 0 стоил ~9 нс стековой
   записи, которых ступень 1 не платила. Один скалярный сток, который платят ВСЕ ступени, порядок
   инвертировать не может.

Плюс планка эталона: ранняя форма брала абсолют bevy отдельным блоком после всех наших проходов и
печатала **0.47×** там, где чередование внутри окна читает **0.65×**. Абсолюты между окнами не
сравнивают — правило известно, но зонд, написанный «в лоб», нарушает его молча.

### Предмет 1 — спавн: 46 % цены сидело в ВЫДАЧЕ ID, а не там, где искали

Лестница на бандле самой ячейки (4 компонента, 100 Б), ns/спавн:

| ступень | было | стало |
|---|---|---|
| конструктор бандла | 1.8 | 1.8 |
| **`entities.allocate`** | **+30.2** | **+7.0** |
| `get_location` (касание 1) | +0.6 | +1.0 |
| `ensure_record` (касание 2) | −0.6 | +0.2 |
| кэш бандла + скретч колонок | +3.4 | +3.0 |
| `entities.push` архетипа | +3.0 | +1.9 |
| запись данных компонентов | +21.8 | +26.6 |
| ячейки тиков (2 на компонент) | +8.6 | +7.6 |
| агрегаты тиков | +0.8 | +5.9 |
| `set_location` (касание 3) | −1.4 | −5.5 |
| **настоящий `World::spawn`** | **66.0** | **46.4** |

**Гипотеза плана — «три касания таблицы сущностей против двух у bevy» — ОПРОВЕРГНУТА:** касания
стоят в пределах шума. Гипотеза «два `Vec::push` тиков на компонент» подтвердилась, но оказалась
ВТОРОЙ статьёй (4.1 нс на компонент по подгонке ширины), а первой — `allocate` с шестью атомарными
операциями на сущность. Разбор и фикс — ALLOC-LEASE-0830.

Поворот лестницы по ШИРИНЕ (то, чего ячейка не умеет — она держит ширину 4 фиксированной) дал
разделение «на сущность / на компонент», и без него `allocate` не был бы опознан как главный:
на ширине 8 его 30 нс разбавлены до 25 % кадра, на ширине 1 это 65 %. Планка чередованием с bevy
по ширинам 1/2/4/8: **0.65/0.78/0.87/0.94 → 1.36/1.39/1.17/1.03**.

### Предмет 2 — `commands_insert`: дорога только ЗАПИСЬ, и её цена разложена надвое

Вычитание двух ячеек говорило «дело в машинерии Commands». Измерение говорит точнее (10k вставок,
всё внутри одного окна, ns/вставку):

| | apex | bevy | ×bevy |
|---|---|---|---|
| прямой `World::insert` | 35.2 | 43.6 | **1.24 впереди** |
| половина ЗАПИСИ | 21.2 | 7.4 | **0.35 позади** |
| половина ПРИМЕНЕНИЯ | 36.0 | 48.2 | **1.34 впереди** |
| отложенный путь целиком | 57.2 | 55.6 | 0.97 |

Внутри записи: `arena.alloc` 3.2, `queue.push` **16.1**. Два варианта, снятые чередованием друг с
другом, делят 16.1 надвое и называют обе половины: **6.7 нс — перерост аллокации**, которую
`apply` выбрасывает каждый раз (в зарезервированную очередь 13.2 против 19.9), и **~7.5 нс —
ширина самой записи** (в 12-байтовую вместо 48-байтовой 5.7).

Форма решения — та же, что у BUNDLE-RESOLVE-0830, в другой проекции: три функциональных указателя
на КАЖДОЙ команде это свойство ТИПА. Записано как **CMD-RECORD-0830**, с оговоркой, которая важнее
самой правки: **половину «ёмкость» ячейка не увидит** — `commands_insert` заводит `Commands::new()`
на каждый прогон, а выигрыш живёт там, где `Commands` переживает кадр. Мерить придётся зондом с
рукой «переиспользованный `Commands`», иначе правка выйдет одновременно бесплатной и бесполезной.

**Взято и закрыто тем же заходом.** Обе половины, в форме эталона и на наших козырях:
ширина — пер-типовая константа `&'static InsertVtable`/`&'static SpawnVtable` (то же, чем
`BundleCache` лечил спавн: ответ, принадлежащий ТИПУ, не пишется на каждый вызов) плюс `Box` для
редких широких вариантов, `size_of::<Command>()` **48 → 32**; ёмкость — `apply` меняет живую
очередь на запасной буфер и вычитывает команды сырым указателем после `set_len(0)`, так что
аллокация возвращается, а реэнтрантный `push` попадает в очередь, которую проход не обходит.

| | было | стало | bevy |
|---|---|---|---|
| запись, ХОЛОДНЫЙ `Commands` | 21.2 | 18.5 | 7.4 |
| **запись, ВТОРОЙ кадр** | 21.2 | **7.5** | 7.4 |
| обход очереди в применении | 12.4 | **2.8** | — |
| применение целиком | 36.0 | 36.7 | 49.3 |
| **отложенный путь целиком (тёплый)** | — | **1.20× впереди** | — |

Зонд по дороге предъявил ещё один свой дефект и он записан: его копия `apply` не последовала за
производством и продолжала обходить очередь через `into_iter().peekable()` — строка `copy drift`
показала −5.9 нс, то есть настоящий путь ДЕШЕВЛЕ копии. Копия доведена, дрейф −0.1. И строка
«вердикт» зонда, делившая разность разностей, выдавала 1.83× / 2.12× / 3.26× / 4.26× на четырёх
прогонах ОДНОГО бинаря, пока каждая её половина стояла в пределах нескольких процентов, — величина,
чей разброс есть её вердикт, судьёй быть не может (урок 25.4). Заменена на половины.

---

## Внеплановая работа 2026-09-07 — рефлективная дверь несла СВОЙ контекст вместо контекста вызывающего

**Заведено движком по цене потерянной авторской работы** (apex-engine TD-467; нашёл пользователь
редактора: «иногда с объёма тумана слетает текстура»).

`World::component_json` / `set_component_json` / `apply_component_json` зашивали `&mut NoContext`.
Для компонента, чей документный вид совпадает с оперативным, это верно. Для компонента, чей
документный вид есть ССЫЛКА (путь к ассету там, где живое значение держит хэндл), назвать и
разрешить её умеет только контекст, знающий проект, — и через зашитый `NoContext` такой компонент
не переживал ни одну из трёх дверей: чтение возвращало опустошённую ссылку, запись клала пустоту
обратно.

**Дороже всех третья дверь:** она единственная ЧИТАЕТ И ПИШЕТ одним жестом, а частичное значение
молчит о полях, которых не называет, — поэтому опустошённая в базе ссылка переживала слияние и
уезжала в мир. Автор правил одно число и терял ассет, которого не касался.

Взято (`decisions/ADR-013`): у каждой двери появился `*_with(…, ctx)`, бесконтекстные делегируют в
них с `NoContext`. Обязательным сделано не наличие контекста, а ВОЗМОЖНОСТЬ его передать: до правки
корректный вызывающий не мог поступить правильно, как бы ни старался. Что значит «меня нельзя
читать без контекста» — остаётся ответом СЕРДЕ компонента; ядро не знает, что такое ассет.

Гейт `a_reflective_door_carries_the_callers_serde_context` проходит тот самый жест пользователя —
частичную правку одного не-ссылочного поля; на инъекции даёт `Ref { id: None, other: 42 }` против
`Ref { id: Some(7), other: 42 }`, ровно форму утраченной текстуры. Набор ядра 618/618.

## Внеплановая работа 2026-09-07 (вторая) — зонд формы платил вводом-выводом за вопрос, которого автор не задавал

**Заведено движком по цене 🔴-долга** (apex-engine TD-465 и TD-471; второй стоял красным как
«щелчок в списке пишет чужие значения в чужое свойство» — и записей не было ни одной).

Рефлективный инспектор движка узнаёт ФОРМУ узла единственным доступным способом: подставляет
кандидата и прогоняет дерево через настоящий `deserialize_fn` типа, читая, что вернулось. Ничто
другое не отличит `u8` от `f32`, `{"Px": 3}` от `Val::Percent`, перечисление от `String`.

Компонент, чей документный вид — ССЫЛКА, разрешал каждого такого кандидата против проекта: **один**
просмотр `FogVolume` дал **45** предупреждений «texture reference `Bokeh` could not be loaded» и 45
неудачных обращений к диску. Имена в них (`Bokeh`, `Monday`, `1970-01-01`, пустая строка) — весь
словарь перечислений редактора, предложенный узел за узлом. Обе половины цены настоящие: чтения, и
предупреждение, которое кричит 45 раз и потому перестаёт значить хоть что-нибудь.

Взято (`decisions/ADR-014`): `SerdeContext` получил ОДИН бит с умолчанием — `probing()` /
`set_probing(on) -> прежнее`. Ядро владеет им потому, что владеет ВЫЗОВОМ: бит говорит, ДЛЯ ЧЕГО
проход, и не называет ничего из того, что резолвит потребитель. Ни один компонент не менялся.
`set_probing` возвращает прежнее значение, а не `()`, — зонд обязан вернуть вызывающему его режим,
а не предположить, что тот был обычным чтением.

Форма выбрана сверкой с эталонами ДО правки: Unity отвечает форму `SerializedProperty`, Unreal —
`FProperty` (и `FSoftObjectPath` намеренно НЕ разрешается, пока не попросят), Godot —
`get_property_list()` с `PROPERTY_HINT_RESOURCE_TYPE`. Все трое: форму отвечает система типов,
разрешение ссылки — отдельный явный акт.

Гейт живёт у ПОТРЕБИТЕЛЯ, потому что обязанность его (движок,
`a_shape_probe_says_nothing_about_the_authors_document_and_still_answers`): 46 предупреждений → 0
при неизменных ответах о форме. Вторая половина гейта обязательна — тишину можно купить, сломав
зонд, и это доказано отдельной инъекцией. В ядре — `the_probe_bit_is_readable_and_restorable_through_the_trait_object`.
