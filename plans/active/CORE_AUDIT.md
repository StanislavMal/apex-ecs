...
- **C5 (S-M, тем же заходом):** on_add/on_remove-хук ChildOf стампит change-tick
  LocalTransform субъекта → reparent диртит поддерево. Перед фиксом проверить сценой движка
  (подозрение может компенсироваться редактором).
