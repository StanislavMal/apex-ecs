...
- **split-par_for_each (§7 headline) — предв. анализ ПЕРЕД реализацией:** текущий heavy_compute
  567µs УЖЕ быстрее потерянного split-варианта (660µs), а бенчи используют РАВНОМЕРНЫЕ архетипы,
  где фикс-чанк + work-stealing `par_iter` ≈ рекурсивный сплиттер. Legion-стиль split выигрывает
  на СКОШЕННЫХ распределениях (мало огромных + много крошечных архетипов), которых в бенчах нет.
  Ожидание: нейтрально на текущем сьюте; для честного A/B нужен скошенный par-бенч. Решать
  замером, не гаданием (следующий шаг волны 5), но НЕ переписывать sound-producer ради вероятно
  нейтрального результата без скошенного бенча.
